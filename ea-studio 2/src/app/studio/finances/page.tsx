...
export default async function FinancesPage() {
  ...
  const [
    ...
    supabase.from('orders').select('*, contacts(name), brands(name,color), products(name)').eq('owner_id', user.id).eq('status', 'active').order('created_at', { ascending: false }).catch(() => ({ data: null })),
    supabase.from('finance_transactions').select('*,brands(name,color)').eq('owner_id', user.id).eq('type', 'in').gte('date', monthStart).lte('date', monthEnd).order('date').catch(() => ({ data: null })),
    supabase.from('finance_transactions').select('*,brands(name,color)').eq('owner_id', user.id).eq('type', 'out').gte('date', monthStart).lte('date', monthEnd).order('date').catch(() => ({ data: null })),
  ])
  ...
  // Target tracking
  const targetMrr = 3000000 // £30,000
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '18px' }}>
            ...
            {(txIn || []).length === 0 ? (
              <div style={{ fontSize: '12px', color: 'var(--dim)' }}>No income recorded this month</div>
            ) : (
              (txIn || []).map(t => (
                <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--border)', fontSize: '12px' }}>
                  <div>
                    ...
                    <div style={{ color: 'var(--dim)', fontSize: '10px' }}>{t.date} · {t.category}</div>
                  </div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '18px' }}>
            ...
            {(txOut || []).length === 0 ? (
              <div style={{ fontSize: '12px', color: 'var(--dim)' }}>No expenses recorded this month</div>
            ) : (
              (txOut || []).map(t => (
                <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--border)', fontSize: '12px' }}>
                  <div>
