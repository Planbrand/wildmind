...
export function NewCampaignButton({ brands, ownerId }: { brands: Brand[]; ownerId: string }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      await createCampaign(ownerId, form.brand_id, form.name)
      setOpen(false)
        ...
        >
          <div style={{ background: 'var(--surface)', borderRadius: '16px', border: '1px solid var(--border)', width: '100%', maxWidth: 440 }}>
            ...
            <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. UK ecom brands — basket builder Q2"
                  style={input}
                  autoFocus
                />
              </div>
              <div>
                ...
                <select value={form.brand_id} onChange={e => set('brand_id', e.target.value)} style={input}>
                  {brands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                </select>
              </div>
              <p style={{ fontSize: '12px', color: 'var(--muted)', margin: 0 }}>
                You'll set up the ICP, email sequence, and lead list next. The campaign won't send until you approve it.
              </p>
            </div>
            <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border)', display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              <button onClick={submit} disabled={isPending || !form.name.trim() || !form.brand_id} style={btnPrimary}>
